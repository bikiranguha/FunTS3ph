    ...
    for line2 in list(HVLineSet):
        ...
        for FaultBus in FaultBusList: # simulate faults on each side of the 2nd line
	        ...
	        for key in Results:
                ...
                for i in range(VmagSize):
                    try:
                        ...
                        continue
                cropped_dv_dt = np.absolute(dv_dt[ind_1sec:ind_1sec+10]) # ten cycles after 2 seconds
                ...
                if dv_dt_mean > dv_dt_threshold:
                    ...
                    LVReportDict[currentEvent].highDvDTBuses.append(key)  # get the bus
            ...
            if currentEvent in LVReportDict:
                ...
                highDVDVBuses = LVReportDict[currentEvent].highDvDTBuses
                for bus in LVBuses:
                    ...
                    plt.ylim(-0.1,1.5)
                    plt.grid()
                    figName = eventdir + '/' +  'Bus'+ Bus+'VMag.png'
                    plt.savefig(figName)
                    plt.close()
                ...
                for bus in highDvDTBuses:
                    ...
                    plt.ylim(-0.5,0.5)
                    plt.grid()
                    figName = eventdir + '/' + 'Bus'+ Bus+'dVDt.png'
                    plt.savefig(figName)
                    plt.close()
...
for event in LVReportDict:
    ...
    highDVDVBusList = LVReportDict[currentEvent].highDvDTBuses
    LVString = event + ';' + 'LV Buses: ' + str(LVBusList)
    outputLines.append(LVString)
    highDvDTString = event + ';' + 'High DVDT Buses: ' + str(highDVDVBusList)
